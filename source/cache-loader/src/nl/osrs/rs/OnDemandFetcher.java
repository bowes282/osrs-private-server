...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
			...
			if (expectedSize > 0 && j >= expectedSize) {
				...
				if (expectedSize + completedSize >= abyte0.length && current != null) {
					if (clientInstance.decompressors[0] != null)
						clientInstance.decompressors[current.dataType + 1].repack(abyte0.length, abyte0, current.ID);
	...
	public void method560(int i, int j) {
		...
		synchronized (onDemandDataList) {
			onDemandDataList.insertHead(onDemandData);
		}
	}
	...
	public void fetch(byte byte0, int index, int record) {
		if (clientInstance.decompressors[0] == null)
			return;
		if (versions[index][record] == 0)
			return;
		clientInstance.decompressors[index + 1].decompress(record);
		fileStatus[index][record] = byte0;
		if (byte0 > anInt1332)
			anInt1332 = byte0;
		totalFiles++;
	}
	...
	public void emptyOnDemandDataList() {
		synchronized (onDemandDataList) {
			onDemandDataList.removeAll();
		}
	}
	...
	private void requestData() {
		while (uncompletedCount == 0 && completedCount < 10) {
			...
			OnDemandData onDemandData;
			synchronized (onDemandDataList) {
				onDemandData = (OnDemandData) onDemandDataList.popHead();
			}
			while (onDemandData != null) {
	...
	private final NodeList onDemandDataList;
