package nl.osrs.rs.definitions;

import nl.osrs.rs.Class36;
import nl.osrs.rs.Client;
import nl.osrs.rs.MRUNodes;
import nl.osrs.rs.Model;
import nl.osrs.rs.Stream;
import nl.osrs.rs.StreamLoader;
import nl.osrs.rs.VarBit;
...
public final class EntityDef {
	...
	public Model method160() {
		...
		if (entityModelIds == null)
			return null;
		boolean flag1 = false;
		for (int i = 0; i < entityModelIds.length; i++)
			if (!Model.method463(entityModelIds[i]))
				flag1 = true;

		if (flag1)
			return null;
		Model aclass30_sub2_sub4_sub6s[] = new Model[entityModelIds.length];
		for (int j = 0; j < entityModelIds.length; j++)
			aclass30_sub2_sub4_sub6s[j] = Model.method462(entityModelIds[j]);
	...
	public void readValues(Stream stream) {
		do {
			...
			if (i == 1) {
				...
				entityModelIds = new int[l];
				for (int l1 = 0; l1 < l; l1++)
					entityModelIds[l1] = stream.readUnsignedWord();
