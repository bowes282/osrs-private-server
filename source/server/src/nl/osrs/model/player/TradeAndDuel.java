...
public class TradeAndDuel{
	...
	**/
	...
	public boolean fromTrade(int itemID, int fromSlot, int amount) {
		...
		o.tradeConfirmed = false;
			if(!ItemWearing.itemStackable[itemID]) {
	...
	public boolean tradeItem(int itemID, int fromSlot, int amount) {
		...
		c.tradeConfirmed = false;
		o.tradeConfirmed = false;
		if(!ItemWearing.itemStackable[itemID] && !ItemWearing.itemIsNote[itemID]) {
			for(int a = 0; a < amount; a++) {
		...
		for (GameItem item : offeredItems) {
		    if (item.id > 0) {
				if (item.amount >= 1000 && item.amount < 1000000) {
					...
					if(Count == 0) {	
						SendTrade = ItemHandler.getItemName(item.id);		
					} else {
						SendTrade = SendTrade + "\\n" + ItemHandler.getItemName(item.id);
					}
	...
	**/
	...
	public void confirmDuel() {
		...
		String itemId = "";
		for(GameItem item : stakedItems) {
			if(ItemWearing.itemStackable[item.id] || ItemWearing.itemIsNote[item.id]) {
				itemId += ItemHandler.getItemName(item.id) + " x " + Misc.format(item.amount) +"\\n";
			}  else  {
				itemId += ItemHandler.getItemName(item.id) + "\\n";
			}
		}
	...
	public void claimStakedItems() {
		for(GameItem item : otherStakedItems) {
			if(item.id > 0 && item.amount > 0) {
				if(ItemWearing.itemStackable[item.id]) {
